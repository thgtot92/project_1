...
→ 예산 제약 시 최우선 설치 대상
""",
    # 25: 흑석동 한정 예산 최적화 (보도 위 신규)
    25: """[25p · 흑석동 한정 예산 최적화 · 60초]
설정 강화 (사용자 피드백 반영 — '산같은 녹지에 추천되는 문제'):
- WALKABLE_HIGHWAYS 에서 path/steps 제거 (산책로·등산로 컷)
- OSMnx 녹지·공원·산림(현충원 포함) 영역 cut 필터 신설
- intersection_density 가중치 0.20 → 0.40 (교차로·횡단보도 최강조)
- 흑석동 한정 + 기존 그늘막 50m 회피 유지

후보 풀: 640(흑석동 안) → 7(필터) → 4(그늘막 회피)

배낭 ILP 결과 (PuLP CBC, Optimal):
- 예산 4천만원 / 단가 800만원 → 4개 선정 (예산 80%)
- 잔여 800만원 — 정책 메시지 강화

선정 4곳 (모두 보도 위, 사각지대·중간):
- #1 (37.51138, 126.96120) — 사각지대 234.8m
- #2 (37.50328, 126.96465) — 사각지대 153.8m
- #3 (37.50146, 126.96014) — 중간 51m (보강)
- #4 (37.50509, 126.96577) — 중간 70m (보강)

핵심 메시지:
- 보도 위 신규 위치는 흑석동에 4곳 한계 = 정책: 흑석동 충분, 다른 동 우선
- intersection_density 0.40 최강조 → 진짜 보행 동선만 추천

예상 질문:
- 왜 5개에서 4개로 줄었나? → 녹지·산책로 컷 + 보도 강제로 후보 자체가 줄어듦 (정확도 우선)
""",
    # 26: 결론 + 다음 단계
    26: """[26p · 결론 · 45초]
한 줄 요약:
...
def build_p25_blindspot_budget(slide):
    """[25p] 흑석동 한정 예산 최적화 — 보도 위 신규 위치 4곳 (한계)."""
    _header(slide, "흑석동 한정 예산 최적화",
            "보도 위 신규 위치만 (녹지·산책로 컷 + 기존 그늘막 50m 회피)")
    ...
    _txt(slide, Inches(0.85), Inches(1.58), Inches(8.3), Inches(0.3),
         "🎯 흑석동 · 보도 10m · 녹지·산책로 제외 · 그늘막 50m 회피 · "
         "intersection_density 가중치 0.40 (최강조) · 후보 640→7→4 · 4 선정",
         size=9, bold=True, color=RGBColor(0xFF, 0xFF, 0xFF))

    # 결과 표 (4개로 축소)
    rows = [
        ("#", "좌표 (lat, lon)",        "score",  "사각지대 거리"),
        ("1", "37.51138, 126.96120",   "0.067",  "234.8m  ⭐사각지대"),
        ("2", "37.50328, 126.96465",   "0.044",  "153.8m  ⭐사각지대"),
        ("3", "37.50146, 126.96014",   "0.031",  "51.0m   ○ 중간"),
        ("4", "37.50509, 126.96577",   "0.001",  "70.0m   ○ 중간"),
    ]
    ...
    _txt(slide, Inches(6.75), Inches(2.30), Inches(2.5), Inches(0.3),
         "📌 정책 함의", size=10, bold=True, color=C_ACCENT)
