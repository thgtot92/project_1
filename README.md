...
다변수 가중합 · 공간 필터 · 시나리오 민감도 + **2종 CV**(항공 SAM + 거리뷰 SegFormer)
+ **흑석동 DSM 누적 그림자**(Deep Umbra 영감, 강의자료 13p 추천) + **OSMnx 교차로 밀도**
+ NGII 실측 건물 3,775동을 통합해 **TOP 10 최적 입지**와 **6개 정책 관점 모두에서
살아남는 강건 입지 2곳**을 도출합니다.

---

## 한 줄 요약

> 항공사진·거리뷰·DSM·도로망까지 4종 데이터를 컴퓨터 비전·GIS로 통합해,
> 6가지 정책 관점 모두에서 살아남는 그늘막 강건 입지 **2곳**을 찾아냈다.

---

## 핵심 결과

### 강건 입지 (Robust Location) — 6개 시나리오 공통 추천
...
Score = 0.22·popdens + 0.22·lst + 0.18·vuln
      ...
      + 0.13·streetview_deficit               (CV-B SegFormer)
      + 0.10·intersection_density             (OSMnx 교차로, NEW)
...
| `natural` (−) | 자연 그늘 (건물 그림자) | **CV-A** SAM 30동 + **NGII 흑석동 3,775동** + 오후 3시 / 흑석동은 **CV-DSM** 4시점 누적 (Deep Umbra 영감) |
| `streetview_deficit` (+) | 거리뷰 그늘 결핍 | **CV-B** SegFormer 19-class · `walkable × (1−building−vegetation)` |
| `intersection_density` (+) | 교차로 밀도 | **OSMnx** walkable highway → degree≥3 노드 → 반경 80m 내 개수 |
